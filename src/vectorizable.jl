...
@generated function vload(ptr::Ptr{T}, i::I) where {T <: LLVMCompatible, I<:Integer}
    ...
    alignment = Base.datatype_alignment(T)
    # push!(flags, "!noalias !0")
    push!(instrs, "%offsetptr = add nsw nuw $ptyp %0, %1")
    push!(instrs, "%ptr = inttoptr $ptyp %offsetptr to $typ*")
...
struct PackedStridedPointer{T,N} <: AbstractColumnMajorStridedPointer{T,N}
    ptr::Ptr{T}
    "Strides 2:N of N-d array in bytes."
...
struct RowMajorStridedPointer{T,N} <: AbstractRowMajorStridedPointer{T,N}
    ptr::Ptr{T}
    "Strides 1:N-1 of N-d array in bytes, in reversed order."
    strides::NTuple{N,Int}
end
...
struct SparseStridedPointer{T,N} <: AbstractSparseStridedPointer{T,N}
    ptr::Ptr{T}
    "Strides 1:N of N-d array in bytes."
    strides::NTuple{N,Int}
end
...
"""
X contains the strides, in bytes.
"""
...
@inline offset(::AbstractColumnMajorStridedPointer{T}, i::Tuple{I}) where {T,I} = @inbounds scalar_mul(i[1], sizeof(T))
@inline offset(ptr::AbstractColumnMajorStridedPointer{T}, i::Tuple{I,Vararg}) where {T,I} = @inbounds i[1] * sizeof(T) + tdot(Base.tail(i), ptr.strides)
@inline offset(ptr::AbstractColumnMajorStridedPointer{T}, i::Tuple{I,Vararg{Integer}}) where {T,I} = @inbounds scalar_mul(i[1], sizeof(T)) + tdot(Base.tail(i), ptr.strides)

@inline offset(ptr::AbstractColumnMajorStridedPointer{T,0}, i::Tuple{I,Vararg{Integer}}) where {T,I} = @inbounds scalar_mul(i[1], sizeof(T))
@inline offset(ptr::AbstractColumnMajorStridedPointer{T,0}, i::Tuple{I,Vararg}) where {T,I} = @inbounds i[1] * sizeof(T)
@inline offset(ptr::AbstractColumnMajorStridedPointer{T,0}, i::Tuple{I}) where {T,I} = @inbounds scalar_mul(i[1], sizeof(T))
@inline offset(ptr::AbstractColumnMajorStridedPointer{T}, i::Integer) where {T} = i * sizeof(T)
@inline offset(ptr::AbstractRowMajorStridedPointer{T}, i::Integer) where {T} = i * sizeof(T)
...
@inline gep(ptr::AbstractStridedPointer, i::Tuple) = ptr.ptr + offset(ptr, i) #gep(ptr.ptr, offset(ptr, i))
@inline gep(ptr::AbstractColumnMajorStridedPointer, i::Tuple{I}) where {I} = gep(ptr.ptr, i[1])
@inline gep(ptr::AbstractStridedPointer, i::Tuple{I}) where {I} = ptr.ptr + first(offset(ptr, i))
...
@inline offset(ptr::AbstractRowMajorStridedPointer{T,0}, i::Tuple{I}) where {T,I} = @inbounds scalar_mul(i[1], sizeof(T))
@inline offset(ptr::AbstractRowMajorStridedPointer{T,1}, i::Tuple{I1,I2}) where {T,I1,I2} = @inbounds i[1]*ptr.strides[1] + i[2] * sizeof(T)
@inline offset(ptr::AbstractRowMajorStridedPointer{T,2}, i::Tuple{I1,I2,I3}) where {T,I1,I2,I3} = @inbounds i[1]*ptr.strides[2] + i[2]*ptr.strides[1] + i[3] * sizeof(T)
@inline offset(ptr::AbstractRowMajorStridedPointer{T}, i::Tuple) where {T,N} = offset(transpose(ptr), reverse(i))

@inline offset(ptr::AbstractRowMajorStridedPointer{T,1}, i::Tuple{I1,I2}) where {T,I1<:Integer,I2} = @inbounds i[1]*ptr.strides[1] + scalar_mul(i[2], sizeof(T))
@inline offset(ptr::AbstractRowMajorStridedPointer{T,2}, i::Tuple{I1,I2,I3}) where {T,I1<:Integer,I2<:Integer,I3} = @inbounds i[1]*ptr.strides[2] + i[2]*ptr.strides[1] + scalar_mul(i[3], sizeof(T))
...
@inline Base.stride(ptr::AbstractColumnMajorStridedPointer{T}, i) where {T} = isone(i) ? 1 : @inbounds ptr.strides[i-1] ÷ sizeof(T)
@inline Base.stride(ptr::AbstractSparseStridedPointer{T}, i) where {T} = @inbounds ptr.strides[i] ÷ sizeof(T)
@generated function Base.stride(::AbstractStaticStridedPointer{T,X}, i) where {T,X}
    strides = map(x -> x ÷ sizeof(T), X.parameters)
    Expr(:block, Expr(:meta, :inline), Expr(:getindex, Expr(:tuple, strides...), :i))
end
@inline LinearAlgebra.stride1(ptr::AbstractColumnMajorStridedPointer) = 1
@inline LinearAlgebra.stride1(ptr::AbstractSparseStridedPointer{T}) where {T} = @inbounds first(ptr.strides) ÷ sizeof(T)
@inline LinearAlgebra.stride1(::AbstractStaticStridedPointer{T,<:Tuple{X,Vararg}}) where {T,X} = X ÷ sizeof(T)
...
@inline Base.setindex!(ptr::AbstractPointer, v, i) = vstore!(ptr, v, (i, ))
@inline Base.setindex!(ptr::AbstractPointer, v, i, j) = vstore!(ptr, v, (i, j))
@inline Base.setindex!(ptr::AbstractPointer, v, i, j, k) = vstore!(ptr, v, (i, j, k))
@inline Base.setindex!(ptr::AbstractPointer, v, i, j, k, rests...) = vstore!(ptr, v, (i, j, k, rests...))
...
@inline Base.setindex!(ptr::AbstractPointer{T}, v::T, i::Tuple) where {T} = vstore!(ptr.ptr, v, offset(ptr, i))
...
@inline stridedpointer(A::AbstractArray{T}) where {T} = PackedStridedPointer(pointer(A), staticmul(tailstrides(A), sizeof(T)))
...
@inline stridedpointer(A::SubArray{T,N,P,S}) where {T,N,P,S <: Tuple{Int,Vararg}} = SparseStridedPointer(pointer(A), staticmul(strides(A), sizeof(T)))
@inline stridedpointer(A::SubArray{T,N,P,S}) where {T,N,P,S} = PackedStridedPointer(pointer(A), staticmul(tailstrides(A), sizeof(T)))
@inline stridedpointer(B::Union{Adjoint{T,A},Transpose{T,A}}) where {T,A <: AbstractVector{T}} = stridedpointer(parent(B))
@inline stridedpointer(A::PermutedDimsArray) = SparseStridedPointer(pointer(A), staticmul(strides(A), sizeof(T))) # TODO: add permutedimsarray, perhaps replacing row major

@inline function stridedpointer(B::Union{Adjoint{T,A},Transpose{T,A}}) where {T,N,A <: AbstractArray{T,N}}
    pB = parent(B)
    RowMajorStridedPointer(pointer(pB), staticmul(tailstrides(pB), sizeof(T)))
end
@inline function stridedpointer(B::Union{Adjoint{Bool,A},Transpose{Bool,A}}) where {T,N,A <: BitArray{N}}
    pB = parent(B)
    RowMajorStridedBitPointer(pointer(pB.chunks), staticmul(tailstrides(pB), sizeof(T)))
end
@inline function stridedpointer(C::Union{Adjoint{T,A},Transpose{T,A}}) where {T, P, B, A <: SubArray{T,2,P,Tuple{Int,Vararg},B}}
    pC = parent(C)
    SparseStridedPointer(pointer(pC), staticmul(reverse(strides(pC)), sizeof(T)))
end
...
@inline function stridedpointer_for_broadcast(A::AbstractArray{T,N}) where {T,N}
    PackedStridedPointer(pointer(A), filter_strides_by_dimequal1(Base.tail(size(A)), staticmul(tailstrides(A), sizeof(T))))
end
...
@inline function stridedpointer_for_broadcast(A::SubArray{T,N,P,S}) where {T,N,P,S <: Tuple{Int,Vararg}}
    SparseStridedPointer(pointer(A), filter_strides_by_dimequal1(size(A), staticmul(strides(A), sizeof(T))))
end
